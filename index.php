...
function bindHandlers(){
  document.querySelectorAll('.row').forEach(row=>{
    row.addEventListener('click', e=>{
      if (e.target.closest('[data-nocollapse]') || e.target.closest('.toggle') || e.target.closest('.round') || e.target.closest('.btn-logs')) return;
      row.classList.toggle('expanded');
      row.querySelector('.chev').textContent = row.classList.contains('expanded') ? '▾' : '▸';
    });
  });
  ...
  document.querySelectorAll('.toggle').forEach(tg=>{
    tg.addEventListener('click', async e=>{
      ...
      const action = tg.classList.contains('on') ? 'disable' : 'enable';
      const res = await fetch('service-api.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ unit, action }) });
      const data = await res.json();
      tg.classList.toggle('on', data.enabled);
    });
  });

  document.querySelectorAll('.round').forEach(btn=>{
    btn.addEventListener('click', async e=>{
      const unit = btn.closest('.row').dataset.unit;
      const action = btn.dataset.action==='play'?'start':btn.dataset.action==='stop'?'stop':'restart';
      const res = await fetch('service-api.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ unit, action }) });
      const data = await res.json();
      btn.closest('.row').querySelector('.cell:nth-child(4)').innerHTML = statusDot(data.status);
    });
  });
  ...
  document.querySelectorAll('.btn-logs').forEach(btn=>{
    btn.addEventListener('click', async e=>{
      ...
      const res = await fetch('service-api.php', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ unit, action:'logs' }) });
      const data = await res.json();
      document.getElementById('logsTitle').textContent = `${unit.toUpperCase()} Logs`;
      document.getElementById('logsBody').textContent = data.output || 'No logs';
